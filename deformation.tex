...
\begin{algorithmic}
\vspace{1mm}
\Require $P \in \ZZ_q[t][x_0,\ldots,x_n]$ satisfying Assumption \ref{assump:diag}, $\tau \in S(\mathbf{F}_{\mathfrak{q}})$ such that $\overline{R}(\tau) \neq 0$
...
we assume that $\alpha_1 = \tilde{\mathcal{O}}(pN)$, [[TODO, see if we can justify this, or
replace with bound from Alan/Gerkmann]] then $\deg(D), K$ are both 
$\tilde{\mathcal{O}}(p\tilde{N}) = \tilde{\mathcal{O}}(pad^n)$. The remaining $p$-adic precisions 
...
\begin{align*}
\mbox{time: }  &\tilde{\mathcal{O}}(a^3 \log^3(p) d^{3n}), \\
