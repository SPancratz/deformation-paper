...
on work of Dwork, but by rewriting and slightly generalizing his formulas we 
...
The entries of $\Phi_0$ can be expressed in terms of the $\alpha_{u,v}$ as follows.
...
\begin{proof} First, let $\QQ_p(\pi)$ denote the totally ramified extension of $\QQ_p$ with $\pi^{p-1} = -p$ 
...
we can write
\begin{align*}
\exp\Bigl(\pi \bigl(P_0(x_0,\ldots,x_n)- P_0(x_0^p,\ldots,x_n^p)\bigl) \Bigr) &= \prod_{i=0}^n \exp \bigl( \pi a_i (x_i^d - x_i^{dp}) \bigr)
\end{align*}
and we also have $D_i =x_i \partial_i + (\pi a_i d) x_i^d$. For $0 \leq i \leq n$, we denote
\begin{align*}
\QQ_p(\pi) \langle x_i \rangle^{\dag} =  \Bigl\{ \sum_{j=0}^{\infty} b_j x_i^{j} \; : \; 
b_{j} \in \QQ_p(\pi), \; \exists c > 0 \text{ s.t.}  
\lim_{j \rightarrow \infty} \bigl(\ord_p(b_j) - c \cdot j \bigr) \geq 0 \Bigr\}
\end{align*}
and we define $\psi_p$ and $D_i$ on $\QQ_p(\pi) \langle x_i \rangle^{\dag}$ by the same formulas as above. Then
\begin{align*}
\psi_p \Bigl( \exp \bigl(\pi a_i(x_i^d- x_i^{pd})\bigr) x_i^{v_i+1} \Bigr) &= \sum_{\substack{m \geq 0 \text{ s.t. }\\ p \mid dm+v_i+1}}  &&\Bigl(\sum_{j=0}^{\floor{m/p}} \frac{(-1)^j (\pi a_i)^{m-(p-1)j}}{j! (m-pj)!} \Bigr) x_i^{(dm+v_i+1)/p}, \\
									   &= \; \; \; \; \; \sum_{m,r} &&\Bigl(\; \sum_{j=0}^{r} \; \; \frac{(-1)^j (\pi a_i)^{m-(p-1)j}}{j! (m-pj)!} \Bigr) x_i^{u_i+1+dr},
\end{align*}
...
\end{align*}
Hence the series $\psi_p \Bigl( \exp \bigl(\pi a_i(x_i^d- x_i^{pd})\bigr) x_i^{v_i+1} \Bigr)$ is congruent to
\begin{align*}
(\pi a_i)^{(p (u_i + 1) - (v_i + 1))/d} 
    \biggl( \sum_{m,r} \Bigl( \frac{u_i+1}{d} \Bigr)_r 
        \sum_{j=0}^{r} \frac{(p a_i^{p-1})^{r-j}}{(m-pj)!j!} \biggr) x_i^{u_i+1}
\end{align*}
modulo $D_i \QQ_p(\pi) \langle x_i \rangle^{\dag}$, where the sum is still over all integers $m, r \geq 0$  that satisfy $p(u_i+1)-(v_i+1)=d(m-pr)$. 
...
by $\sum_{i=0}^n D_i \mathcal{L}^{(i)}$, completing the proof.
\end{proof}

\begin{rem}
The computation in the proof of Theorem~\ref{thm:01-03-diagfrob} essentially goes back to Dwork~\citep[\S 4]{Dwork1964}. He obtained a formula similar to ours, 
that Lauder \citep{Lauder2004b} and Gerkmann \citep{Gerkmann2007} have used in their computations. However, our approach is different in two ways.
...
the computation of $\Phi_0$ (even of the whole deformation algorithm) by a factor $p$, as we will see in Section~\ref{sec:Complexity}. Second, Dwork's formula 
is only valid when the $a_i$ are Teichm\"uller lifts (i.e. $a_i^{p-1}=1$). From a theoretical point of view, of course one can always choose the $a_i$ to be 
...
\begin{equation*}
\ord_p\Bigl(\frac{j}{d}\Bigr)_r \geq \ord_p(r!) 
...
\begin{align*}
\ord_p \Bigl( \sum_{j=0}^{\floor{m/p}} \frac{(p a_i^{p-1})^{\floor{m/p}-j}}{(m-pj)!j!} \Bigr) \geq \frac{p-1}{p^2}m + \floor{\frac{m}{p}}-\frac{m}{p-1}.
\end{align*}
\end{prop}
\begin{proof}
...
\lambda_m &= \sum_{j=0}^{\floor{m/p}} \frac{(-1)^j (\pi a_i)^{m-(p-1)j}}{(m-pj)!j!} \\
          &= (-1)^{\floor{m/p}} (\pi a_i)^{m-(p-1)\floor{m/p}} \Bigl( \sum_{j=0}^{\floor{m/p}} \frac{(p a_i^{p-1})^{\floor{m/p}-j}}{(m-pj)!j!} \Bigr).
\end{align*}
...
\begin{thm} Let $\alpha_{u,v}$ be defined as in Definition~\ref{defn:alpha}. Then
\begin{align*}
0 \leq \ord_p(\alpha_{u,v}) \leq \ord_p\Bigl( \frac{(k(v)-1)!}{(k(u)-1)!} \Bigr)+(n-k(u))+\delta,
\end{align*}
...
\frac{p-1}{p^2}m + \floor{\frac{m}{p}}-\frac{m}{p-1} + \ord_p\Bigl(\floor{\frac{m}{p}}!\Bigr)>-1,
\end{align}
for all integers $m \geq p$. From Proposition~\ref{prop:rfac} and the fact that $m \geq p \floor{m/p}$, it follows that
\begin{align*}
\frac{p-1}{p^2}m + \floor{\frac{m}{p}}-\frac{m}{p-1} + \ord_p\Bigl(\floor{\frac{m}{p}}!\Bigr) \geq \frac{p-1}{p} \floor{\frac{m}{p}} - \floor{\log_p\Bigl(\floor{\frac{m}{p}}\Bigr)+1}.
\end{align*}
...
we have
\begin{equation*}
-\delta \leq \ord_p\Bigl(\frac{(k(v) - 1)!}{(k(u) - 1)!} \Bigr)+ (n-k(u)) - \ord_p( \alpha_{u,v}),
\end{equation*}
...
\[
N_{\Phi_0}' = N_{\Phi_0} + (n-k(u)) + \ord_p\Bigl(\frac{(k(v) - 1)!}{(k(u) - 1)!} \Bigr) + \delta.
\]
\end{cor}
...
\begin{align*}
\ord_p \Bigl( \Bigl( \frac{u_i+1}{d} \Bigr)_r \sum_{j=0}^{r} \frac{(p a_i^{p-1})^{r-j}}{(m-pj)!j!} \Bigr) &\geq  \frac{p-1}{p^2}m + \floor{\frac{m}{p}}-\frac{m}{p-1}+\frac{r}{p-1}-\floor{\log_p(r)+1} \\
...
\item[] \begin{compactenum}[{\hspace{1em}} 1.]
        ...
        \item $(\Phi_0)_{u,v} \gets (-1)^{k(v)} \bigl( \frac{ (k(v)-1)!}{(k(u)-1)!} \bigr) p^{n-k(u)} \alpha_{u,v}^{-1}$ with $p$-adic precision $N_{\Phi_0}$.
      \end{compactenum}   
...
\end{align*}
is equal to the coefficient of $x^m$ in the power series expansion of $\exp(\pi a_i(x-x^p))$. However,
...
\]
so computing of all $b \in \BigOh(d^n)$ elements $\alpha_{u,v}$ takes time 
$\SoftOh(a^2 p^2 d^{3n} \log^2(d_t))$. Hence $\Phi_0$
can be computed in time $\SoftOh(a^2 p d^{2n} (p + d^n) \log^2(d_t))$.

The required space is dominated by storing the power series expansions of $\exp(\pi a_i(x-x^p))$ modulo $x^{\mathcal{M}+1}$ for $0 \leq i \leq n$ and
is there given by
\[
