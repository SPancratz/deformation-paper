...
functions of order greater than or equal to $-(\alpha_1+p \alpha_2)$ at the 
point $z$. 
\end{thm}
...
\]
at the point $z$. Here, $\alpha_1, \alpha_2$ are defined as in 
...
\end{eqnarray*}
Let $i \in \NN$ be maximal such that
\begin{align*}
i+(n-1)+\ord_p ((n-1)!)-n \floor{\log_p(p(n+i)-1)} < N,
\end{align*}
and define
\[
\beta = \begin{cases}
...
order greater than or equal to $\beta$ at the point $z$.
\end{thm}
...
considering as input variables $n$, $d$, $p$, and $a = \log_p \mathfrak{q}$. 
...
in $\SoftOh(k^{\omega})$ ring operations. An invertible 
$k \times k$ matrix can then be inverted in $\SoftOh(k^{\omega})$ 
ring operations as well. Moreover, it is known that $2 \leq \omega \leq 2.3727$. 

We now turn to two specific $p$-adic operations.  First, images of elements of 
...
We begin by estimating the various precisions that we require.  We denote 
$a' = \log_p q$, noting that $a'$ divides $a$, and also let $d_t$ denote 
the degree of $P$ in the variable~$t$.

\begin{prop}
All $p$-adic precisions that we use lie in $\SoftOh(a d^n)$.
\end{prop}
...
$\SoftOh(p\tilde{N}) \subset \SoftOh(p a d^n)$ [[TODO Jan, wrong]].
...
are also all $\SoftOh(a d^n)$ by Remark~\ref{rem:precgm}, 
Corollary~\ref{cor:Ntilde}, Theorem~\ref{thm:Ni}, Remark~\ref{rem:prectau}
and Remark~\ref{rem:workprecchi}, noting 
that the logarithms that appear in the theorem are to base~$p$.
\end{proof}
...
\begin{align*}
\mbox{time: }  &\SoftOh(a a' \log(p) \max \{ d^{n(\omega+2)} e^{n(\omega+1)}, d^{5n}e^{3n} \} d_t), \\
...
operations in the ring $\QQ_q[t, R(t)^{-1}]$ to precision~$\SoftOh(a d^n)$. 
As there are $\BigOh(n)$ of these matrices, this takes time 
\[
\SoftOh((de)^{n \omega} (a' \log(p)) (a d^n) (de)^n d_t) = 
...
\[
\SoftOh((de)^{2n} (a' \log(p)) (a d^n) (de)^n d_t) = 
    \SoftOh(a a' \log(p) d^{4n}e^{3n} d_t),
\]
because of the quadratic compexity of the matrix-vector product.  
...
\mbox{space: } &\SoftOh(a^2 \log(p) d^{2n}).
\end{align*}
\end{prop}

\begin{proof}
Let $\tilde{N}_{\Phi_0} \in \SoftOh(a d^n)$ denote the $p$-adic 
...
takes time $\SoftOh(\tilde{N}_{\Phi_0}^3p) = \SoftOh(a^3 p d^{3n})$.
...
$\SoftOh(\tilde{N}_{\Phi_0}^2 \log(p)) = \SoftOh(a^2 \log(p) d^{2n})$.
\end{proof}

\begin{rem} \ 
[[TODO:  Point out that this approach is novel and for this part of the 
computation reduces the dependence on $p$ from $p^2$ to $p$ when compared 
with Lauder/ Gerkmann.  And that this is a trade-off in that the dependence 
on the precision turns quasi-cubic instead of quasi-quadratic because of 
this.  Finally, say this does not matter in the context of the deformation 
method and hence this is a straight improvement, reducing the dependence on 
the prime $p$.]]
\end{rem}
...
\begin{prop}
Assume that the matrices $M = G/r$, $\Phi_0$ have been computed already
in Algorithm~\ref{alg:expansion}.  The subsequent computation of the 
power series expansion of the matrix~$\Phi$ then takes
\begin{align*}
...
We compute the matrix $C = \sum_i C_i t^i$ as a sequence of $K$~matrices 
according to Equation~\eqref{eq:recursiondifeq}.  As each step is dominated 
by the $\deg(B) \in \SoftOh((de)^n d_t)$ matrix products, the matrix~$C$ 
can be computed in time 
\begin{equation*}
\SoftOh\bigl(K (de)^n d_t b^{\omega} (a' \log p) (a d^n)\bigr) 
    \subset \SoftOh\bigl(d^{n(\omega + 3)} e^n d_t a' a^2 p\bigr).
\end{equation*}
We now consider the complexity of computing the matrix $\sigma(C)^{-1}$, 
assuming that we compute the matrix~$C^{-1}$ as the solution to 
Equation~\eqref{eq:01-GMDE-Dual}, then apply the map $\sigma$ 
to each of its entries, and finally apply the map $t \mapsto t^p$.  The 
first step takes time 
\begin{equation*}
\SoftOh\bigl( \tfrac{K}{p} (de)^n d_t b^{\omega} (a' \log p) (ad^n) \bigr)
    \subset \SoftOh\bigl( d^{n(\omega+3)} a^2 a' d_t e^n \log p \bigr), 
\end{equation*}
the second step takes 
\begin{equation*}
\SoftOh\bigl( \tfrac{K}{p} b^2 \bigl( a d^n d^{2n} (\log^2(p) a' + a' a d^n \log p) \bigl) \bigr)
\subset \SoftOh\bigl( a a' d^{3n} \log(p) (\log p + a d^n) \bigr), 
\end{equation*}
and in light of the computation of the matrix~$C$ before, we can ignore 
the application of the map $t \mapsto t^p$.  Finally, the matrix product 
$C \Phi_0 \sigma(C)^{-1}$ can be computed in time 
\begin{equation*}
\SoftOh\bigl( b^{\omega} K a' (a d^n) \log p \bigr) 
    \subset \SoftOh\bigl( a^2 a' p d^{n(\omega + 2)} \bigr).
\end{equation*}

The space requirement is dominated by the matrix~$C$, which is
\begin{equation*}
\SoftOh(b^2 K \log(p) (a d^n)) = \SoftOh(a^2 a' p d^{4n}). \qedhere
\end{equation*}
\end{proof}
...
\begin{align*}
\mbox{time: }  &\SoftOh\bigl(a^2 a' p d^{4n}\bigr), \\ 
\mbox{space: } &\SoftOh\bigl(a^2 a' p d^{4n}\bigr).
\end{align*}
\end{prop}

\begin{proof}
We first recall that the Teichm\"uller lift $\hat{\tau}$ 
can be computed to precision $\SoftOh(a d^n)$ in time 
$\SoftOh(a^2 d^n \log^2 p)$.  Next, we observe that the 
scalar-matrix product $D(t) \Phi(t) \bmod t^K$ over~$\QQ_q[t]$ 
requires time 
\begin{equation*}
\SoftOh\bigl( b^2 K a' (a d^n) \log p \bigr) 
    \subset \SoftOh\bigl( a^2 a' p d^{4n} \bigr).
\end{equation*}
Finally, we consider the substitution of $\tau \in \ZZ_{\mathfrak{q}}$ 
into the $b^2$ polynomials.  Each of these can be thought 
of as a modular composition of polynomials over $\ZZ_{q}$ 
where the modulus is the defining polynomial for the extension 
$\QQ_{\mathfrak{q}} / \QQ_{q}$ of degree $a/a'$, however, care 
has to be taken to include the additional reduction of the 
polynomials of degree less than~$K$ modulo the defining polynomial, 
too, so that polynomials involved in the composition have degree 
less than~$a/a'$.  Thus, the substitutions require time 
\begin{equation*}
\SoftOh\Bigl( b^{\omega} \bigl(K a' \log(p) (a d^n) + \tfrac{a}{a'} \log(p) a' (a d^n) \bigr)
    \subset \SoftOh(a^2 a' p d^{4n}).
\end{equation*}
Clearly, evaluating and inverting $D(\hat{\tau})$ and performing the 
scalar multiplication can be ignored, and the result now follows.

The space requirement is dominated by the matrix $D(t) \Phi(t) \mod t^K$, 
which has size 
\begin{equation*}
\SoftOh(b^2 K a' \log(p) (a d^n)) \subset \SoftOh(a^2 a' p d^{4n}). 
\qedhere
\end{equation*}
\end{proof}
...
\begin{align*}
\mbox{time: }  & \SoftOh(a d^{2n} \log^2(p) + a^2 d^{n(\omega+1)} \log p), \\
...
\begin{gather*}
\SoftOh\Bigl( b^2 \log a \bigl( a \log^2(p) + a (a d^n) \log p \bigr) 
    + \log(a) b^{\omega} a (a d^n) \log p \Bigr), \\
\intertext{which is also}
\SoftOh\bigl( a d^{2n} \log^2(p) + a^2 d^{n(\omega+1)} \log p \bigr).
\end{gather*}
...
\begin{equation*}
\SoftOh\bigl(b^{\omega} a (a d^n) \log p\bigr)
    \subset \SoftOh\bigl( d^{n(\omega+1)} a^2 \log p\bigr).
\end{equation*}
...
\begin{align*}
\mbox{time: }  &\SoftOh\Bigl( a a' \log(p) d_t \bigl(d^{n(\omega+2)} e^{n(\omega+1)} + d^{5n} e^{n}\bigr) + a^3 p d^{3n} + a^2 a' p d^{n(\omega+3)} e^n d_t \Bigr), \\
\mbox{space: } &\SoftOh\bigl( a a' \log(p) d^{4n} e^{3n} d_t + a^2 a' p d^{4n} \bigr).
\end{align*}
\end{thm}
