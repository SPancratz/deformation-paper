...
\item In the implementation of the computation of $p^{-1} \Frob_p$ 
      on $\Hrig^{n}(\mathcal{U}_0)$ which is quasi-cubic in the 
      precision, the elements of the sequence $(\mu_m)_{m=0}^{M}$ 
...
\item The computation of the $q_2$th-power Frobenius 
      $\Phi^{(q_1)} \sigma(\Phi^{(q_1)}) \dotsm \sigma^{a_2-1}(\Phi^{(q_1)})$, 
      where $a_2 = \log_{q_1} q_2$, can be carried out in parallel.  This can 
      ...
      that $a_2 = 2^k$.  In this notation, the execution of the 
      loops indexed by~$i$ can be parallelised.
      \begin{algorithm}
      \caption{Parallel computation of $q_2^{-1} \Frob_{q_2} | \Hrig^{n}(\mathcal{U}_{t_1})$}
      \label{alg:ParallelProduct}
      \begin{algorithmic}
      \vspace{1mm}
      \For{$i \gets 0$ \textbf{to} $a_2-1$}
