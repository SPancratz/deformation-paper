...
In the rest of this section (and this paper), $\FF_{\mathfrak{q}}$ always denotes a finite extension 
of $\FF_q$, in other words: $\mathfrak{q}=q^f$ for some $f \in \NN$.
...
b=\frac{1}{d} \bigl((d-1)^{n+1} + (-1)^{n+1}(d-1) \bigr).
\end{equation}
\end{thm}
...
\end{cases}, 
\end{equation}
so that all that remains to be determined is $\Hrig^{n-1}(X_{\tau})$. Finally, one uses the excision short exact 
sequence
...
\end{CD} 
\end{equation}
to relate $\Hrig^{n-1}(X_{\tau})$ to $\Hrig^{n}(U_{\tau})$ and complete the proof of \eqref{eq:formulazeta}. The 
proof of \eqref{eq:formulab} on the other hand can be found in [[TODO Jan]]. 
\end{proof}
...
any poles in that residue disk either, for example by Theorem \ref{thm:KedlayaTuitman} below. So the entries 
...
\item Compute the matrix $\Phi_{\tau}^{(a)}$ of the action of $\mathfrak{q}^{-1} \Frob_{\mathfrak{q}}$ (which is also $(p\Frob_p)^a$, 
where $a=\log_p(\mathfrak{q})$) on $\Hrig^n(U_{\tau})$, and use Theorem \ref{thm:hypersurface} to compute the zeta function $Z(X_{\tau},T)$
of the fiber $X_{\tau}$.
\end{enumerate}
...
%TODO mention that Fil^i is the (reverse) Hodge filtration on H^{n-1}(X) with the correct indices
...
basis $\cB$ by
\[
R = \prod_{k=\lfloor n/d \rfloor+1}^{n+1}  \det(\Delta_k),
\]
...
matrix $M$. Recall that $\FF_{\mathfrak{q}}$ denotes a finite extension of $\FF_q$, in
other words: $\mathfrak{q}=q^f$ for some $f \in \NN$.
...
Let $\overline{R} \in \FF_q[t]$ denote the reduction modulo $p$ of the resultant $R$. For every 
...
&\geq -\delta - (n-1).
\end{align*}
Since this holds for an infinite number of $\tau$ (for varying $\mathfrak{q}$), it implies that 
\begin{align*}
\ord_p(\Phi) &\geq -\delta, \\
\ord_p(\Phi^{-1}) &\geq -\delta-(n-1).
\end{align*}
...
Recall from Section \ref{sec:Connection} that $M \in M_{b \times b}(\QQ_q(t))$ denotes the matrix for the Gauss--Manin connection 
...
Note that $r$ is a divisor of the resultant $R$ defined in Definition \ref{defn:resultant}.
...
As we saw in section \ref{sec:Background}, these matrices satisfy the 
...
we also need to compute the matrix $\sigma_p(C)^{-1}$. The matrix~$C^{-1}$ 
...
Since $\ord_p(C) \leq 0$ and $\ord_p(\sigma_p(C)^{-1}) \leq 0$, we have that 
\[
N \geq \ord_p(C) + \ord_p(\Phi_0) + \ord_p(\sigma_p(C)^{-1}).
\]
...
on $\Hrig^{n}(U/S)$, with respect to the basis $[v_1,\dotsc,v_b]$, and let $\delta$ be defined as in Definition
\ref{defn:delta}. 
For $i \in \NN$, put
\begin{align*}
...
our family $X/S$ lying over some $\tau \in S(\FF{\mathfrak{q}})$ with $\mathfrak{q}=q^f$ 
...
$\Phi_{\tau}, \sigma_p(\Phi_{\tau}), \dotsc, \sigma_p^{a-1}(\Phi_{\tau})$ 
...
\]
with $N \geq 0$ and $\delta$ defined as in Definition \ref{defn:delta}. Then
\begin{equation}
...
such that for $\Phi_{\tau}'=W \Phi_{\tau} \sigma_p(W)^{-1}$ we have that $\ord_p(\Phi_{\tau}') \geq 0$. 
...
\begin{proof}
Recall from 
%Theorem \ref{thm:weildeligne} 
% [[TODO Jan]] small problem, U instead of X, easy to fix
that
\[
...
under the map $\alpha \mapsto \mathfrak{q}^{n-1}/\alpha$, we have that
\begin{equation*}
...
\item $\chi(T) \gets \det\bigl(1-T \bigl(\Phi_{\tau} \sigma_p(\Phi_{\tau}) \ldots \sigma_p^{a-1}(\Phi_{\tau}) \bigr)  \bigr) \pmod{p^{N}}$ as an element of $\ZZ[T]$ 
...
\item The computation of the $\mathfrak{q}$th-power Frobenius 
      $\Phi_{\tau} \sigma_p(\Phi_{\tau}) \dotsm \sigma_p^{a-1}(\Phi_{\tau})$, can be carried 
...
[[TODO Seb, should it not be clearer that $\sigma_p^i$ is not computed by composing $\sigma_p$ with itself $i$ times? ]]
