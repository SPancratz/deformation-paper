...
bases $F_k$ and $R_k$, respectively, and let $\pi \colon V_k \onto W_k$ 
denote the projection onto the subspace $W_k \leq V_k$. [[TODO projection
not unique, reformulate]] Then the map 
\begin{equation}
...
\Procedure{Decompose}{$\partial_0 P, \dotsc, \partial_n P, Q$}
...
$i$ we have $p (u_i + 1) \equiv v_i + 1 \pmod{d}$. Then
\begin{equation}
p^{-1} \Frob_p (\omega_1) = 
...
Since $d$ is odd for $p=2$, the numerator is always even and the denominator is odd,
...
suffices to restrict the inner sum in [] to pairs $m,r \geq 0$ such 
that $m \leq M$, or equivalently $r \leq R$, where 
\begin{equation}
