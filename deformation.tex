...
its is better to put $X$ in a family of smooth projective hypersurfaces that 
contains a diagonal hypersurface, first compute the action of the Frobenius map on 
the rigid cohomology of the diagonal hypersurface, and then solve a $p$-adic 
differential equation to obtain the Frobenius map on the rigid cohomology of 
the original hypersurface. He proved that this way one obtains a better time 
complexity in terms of the variables $d,n$.

Actually, this is not entirely correct, because in [[TODO]] Lauder did not work 
with rigid cohomology, but with Dwork cohomology. These two cohomology theories 
...
improvements to just about every step of the algorithm. For example, by using 
recent bounds on the rate of convergence of $p$-adic differential equations, we
obtain much sharper bounds for the $p$-adic and $t$-adic precisions required at
different stages to
obtain provably correct results. The algorithm that we obtain this way has lower 
time and space complexity than Lauder's algorithm. The first author has written a 
very fast C implementation of the algorithm using the library FLINT [[TODO]]. The 
runtimes that we obtain for the examples in [[TODO]] are thousands of times 
lower than the runtimes in that paper. 
