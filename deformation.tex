...
\[
Z(X,T) = \exp \Bigl(\sum_{i=1}^{\infty} |X(\FF_{q^i})| \frac{T^i}{i} \Bigr).
\]
\end{defn}

As we will see in the next section, $Z(X,T)$ 
is a rational function, i.e.\ it is contained in $\QQ(T)$, and 
so can be given by a finite amount of data. Therefore, it is natural 
to ask whether it can be computed effectively. Actually, it 
is not hard to come up with an algorithm. There 
are well known bounds \cite{Bombieri1966} for the degrees of the numerator and 
denominator of $Z(X,T)$, that reduce the computation of $Z(X,T)$ to that 
of a finite number of the $|X(\FF_{q^i})|$, which can be determined
by naive counting. 

A more interesting problem is whether, and if so how, $Z(X,T)$ can be 
computed efficiently. Here efficiently can mean with low time complexity or 
just fast in practice. When $X$ is a (hyper)elliptic curve, this problem
is important for cryptographic applications, and has received
a lot of attention. In this case, very efficient algorithms are available.
For example, when $X$ is an elliptic curve, Schoof's 
algorithm \cite{Schoof1995}, that uses $\ell$-adic \'etale cohomology, has runtime 
polynomial in $\log(q)$, and is also very fast in practice using some 
improvements by Atkins and Elkies.

For more general $X$, the only available option is usually to compute the 
rigid cohomology spaces of $X$, with their natural action of the Frobenius map, 
and then use a Lefschetz formula to deduce the zeta function. This method was
introduced by Kedlaya in the case of hyperelliptic curves in odd characteristic 
\cite{Kedlaya2001}. The same idea has been shown to work in much greater 
generality, for example for smooth projective hypersurfaces \cite{AbbottKedlayaRoe2006}. 

Lauder \cite{Lauder2004a,Lauder2004b} showed that instead of computing the action 
of the Frobenius map on
the rigid cohomology spaces of a smooth projective hypersurface $X$ directly, 
it is better, at least in terms of time complexity, to put $X$ in a family of 
smooth projective hypersurfaces that contains a diagonal hypersurface, first compute 
the action of the Frobenius map on the rigid cohomology of the diagonal 
hypersurface, and then solve a $p$-adic differential equation to obtain the 
Frobenius map on the rigid cohomology of the original hypersurface. 

Actually, this is not entirely correct, because in \cite{Lauder2004a,Lauder2004b} 
Lauder did not work 
with rigid cohomology, but with Dwork cohomology. The two cohomology theories 
are equivalent, but various comparison and finiteness results are more easily 
stated and proved in rigid cohomology. In \cite{Gerkmann2007} Gerkmann reformulated Lauder's
deformation algorithm in terms of rigid cohomology. Moreover, he improved various 
precision bounds, making the algorithm more practical, and he computed a lot of examples.

The aim of this paper is to continue where Lauder and Gerkmann left off. We make 
improvements to almost every step of the algorithm. 
The resulting algorithm has lower time and space complexity than 
Lauder's algorithm. Moreover, the first author has written a publicly
available C implementation of the algorithm using the library FLINT [[TODO]]. 
This implementation lowers the runtime of the examples in \cite{Gerkmann2007} by at 
least three orders of magnitude. So our work is a significant improvement of the work 
of Lauder and Gerkmann.

We now briefly describe the contents of the remaining sections. We
will use some terminology that is introduced only in Section \ref{sec:Background}.
The reader that is not familiar with this terminology should start reading there. 

In Section \ref{sec:Background}, we recall the main theoretical results that we 
will need in the rest of the paper. We limit ourselves to the bare minimum, since 
there are already good references available for the relevant theory \cite{Kedlaya2012}. 
We also introduce a lot of terminology and notation for the rest of the paper. At 
the end of this section we give an overview of the different steps of the deformation 
method. The next four sections each treat one of these steps.
...
in the cohomology. In Algorithm \ref{alg:Connection} we compute the matrix of 
the Gauss--Manin connection. We also prove some lower bounds for the valuation of 
the matrix of Frobenius and its inverse, that are essential for controlling 
$p$-adic precision loss in the Algorithm.

In Section \ref{sec:Diagonal}, we show how to compute the Frobenius matrix of
a diagonal hypersurface over a prime field. Our method is based on a formula of Dwork, but by 
rewriting this formula we obtain Algorithm 
\ref{alg:Diagfrob}, that performs better than the corresponding algorithms 
of Lauder and Gerkmann, both in terms of time complexity and in practice. 

In Section \ref{sec:DifferentialSystem}, we explain how to solve the differential
equation for the Frobenius matrix. We use the same method as Lauder, but 
incorporate improved convergence bounds for $p$-adic differential equations 
by Kedlaya. Our most important result in this section is Theorem \ref{thm:Ni} 
in which we collect the improved precision bounds that follow from our analysis.
In Algorithm \ref{alg:expansion} we compute the power series expansion of the 
Frobenius matrix.

In Section \ref{sec:ZetaFunctions}, we describe how to evaluate the Frobenius
matrix at some fibre, and deduce its zeta function. We combine bounds from various 
sources to bring down the required $p$-adic and $t$-adic precisions 
as much as possible. This results in Algorithm \ref{alg:complete}, which combines
all of our previous algorithms, and is the main result of the paper.

In Section \ref{sec:Complexity}, we analyse the time and space complexity of our 
algorithm and compare these to \cite{Lauder2004a}. 
In Section \ref{sec:Examples}, we compute various numerical examples, and compare 
our runtimes to those in \cite{Gerkmann2007}. 
...
\begin{defn} We define the ring of overconvergent functions
\[
...
to $\QQ_q \langle t, 1/r \rangle^{\dag}$ in the standard way, i.e.\ $\ord_p(f)$
is defined as the maximum, over all ways of writing $f=\sum_{i,j=0}^{\infty} a_{i,j} (t^i / r^j)$,
...
\end{align*}
where $\Phi_0$ is the matrix of $p^{-1}\Frob_p$ on $\Hrig^n(U_0)$ with respect 
to the basis $[e_0,\ldots,e_b]$.
\end{thm}
...
\item Substitute the Teichm\"uller lift $\hat{\tau}$ of an element 
      ...
      which is also equal to $(p^{-1}\Frob_p)^a$, on~$\Hrig^n(U_{\tau})$. 
...
\]
The inclusion on the left is \cite[Lemma 3.4.3]{AbbottKedlayaRoe2006}, and the 
one on the right is \cite[Proposition 3.4.6]{AbbottKedlayaRoe2006}.
...
$[w_1,\ldots,w_b]$, in general we only have that $\ord_p{\Phi'} \geq -\delta'$. 
\end{rem}
...
\begin{align*}
\QQ_q \langle x_0, \ldots, x_n, t, \frac{1}{r}, \frac{1}{P} \rangle^{\dag} = \\
&\Bigl\{ \sum_{i_0,\ldots,i_{n+3}=0}^{\infty} a_{i_0, \ldots, i_{n+3}} 
\frac{x_0^{i_0} \ldots x_n^{i_{n}}t^{i_{n+1}}}{r^{i_{n+2}} P^{i_{n+3}}} \; : \;
a_{i_0, \ldots i_{n+3}} \in \QQ_q, \; \\ 
&\exists c > 0 \text{ s.t.} \lim_{i_0+\ldots+i_{n+3} \rightarrow \infty} \bigl(\ord_p(a_{i_0, \ldots, i_{n+3}} ) - 
c(i_0+\ldots+i_{n+3})\bigr) \geq 0 \Bigr\},
\end{align*}
by putting $\sigma(x_i) = x_i^p$ for $0 \leq i \leq n$. Note that
\begin{align*}
\sigma(\frac{1}{P}) &= P^{-p} \Bigl( 1-\frac{(P^p-\sigma(P))}{P^p} \Bigr)^{-1},
\end{align*}
is an element of this ring, since 
\begin{align*}
...
(x_0 \cdots x_n)^{p-1} \Bigl( \frac{x^{pu} \Omega}{P^{p(k+i)}} \Bigr)
\end{equation*}
...
(x_0 \cdots x_n)^{p-1} \Bigl( \frac{x^{pu} \Omega}{P^{p(k+i)}} \Bigr) \equiv
\frac{\gamma_{i,1} \Omega}{P^1}+\ldots+\frac{\gamma_{i,n} \Omega}{P^n},
\end{equation}
...
For every $\tau \in S(\overline{\FF}_q)$ such that $\overline{R}(\tau) \neq 0$, we
...
\cite[Proposition 3.4.6]{AbbottKedlayaRoe2006}, Equation \eqref{eqn:Froblift}, 
and the inclusion $(n-1)!\Lambda_{\tau,crys} \subset \Lambda_{\tau,\cB}$, that
\begin{align*}
\frac{\gamma_{i,1} \Omega}{P^1}+\ldots+\frac{\gamma_{i,n} \Omega}{P^n} \subset 
