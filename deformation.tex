...
\begin{proof}
TODO Small complication here, Lauder doesn't have factor $\pi^{k_v-k_u}$. 
Bound can be weakened a little bit, but I don't think this necessary.
\end{proof}
...
disk in the complex plane of radius $(b/i) \mathfrak{q}^{i (n-1) / 2}$. Therefore, 
...
\begin{eqnarray*}
\max\{N_i\} \in \BigOh\bigl(a nb \log(b) \bigr) 
            \subset \SoftOh\bigl(a d^n \bigr), \; \; \;
N           \in \SoftOh(a d^n).
\end{eqnarray*}
Hence $\tilde{N}$ from Algorithm~\ref{alg:complete} is $\SoftOh(ad^n)$ 
as well. Now in Theorem~\ref{thm:KedlayaTuitman}, if we assume that 
