...
&\geq -\delta - (n-1).
\end{align*}
By varying $\mathfrak{q}$ we see that these inequalities hold for infinitely 
many~$\tau$ and hence obtain the required bounds.
\end{proof}
...
basis $\cB$. We let $r \in \ZZ_q[t]$ denote a denominator for $M$, such that $M = G/r$, with 
$G \in M_{b \times b}(\QQ_q[t])$, and $r$ divides the resultant $R$ defined in 
Definition \ref{defn:resultant}. By Assumption \ref{assump:diag}, we then have that 
...
respect to the same basis $\cB$, where $\sigma$ is defined as in Definition \ref{defn:sigma}. 
