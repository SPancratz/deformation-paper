...
\item[] \textbf{else}
\item[] \hspace{1em} $\tilde{N} \gets N + \delta$, $\bar{N} \gets N+(a+b-3) \delta$
...
\item Compute $\chi(T) \gets \det\bigl(1-T \bigl(\Phi_{\tau} \sigma(\Phi_{\tau}) \ldots \sigma^{a-1}(\Phi_{\tau}) \bigr)  \bigr)$ to $p$-adic precision $N-\delta$, 
