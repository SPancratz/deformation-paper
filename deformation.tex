...
assuming that we compute the matrix~$C^{-1}$, then apply the map $\sigma$ 
...
ring operations in $\QQ_{\mathfrak{q}}$, taking time 
$\SoftOh((a \log(p))(a \log(p)) (a d^n)) = \SoftOh(a^3 \log^2(p) d^n)$ 
and space $\SoftOh((a \log(p)) (a d^n))$, which is also 
$\SoftOh(a^2 \log(p) d^n)$. 
...
$\SoftOh(b^2 K \log(q) (a d^n)) = \SoftOh(a^2 p \log(q) d^{4n})$.
Then we have to substitute $\hat{\tau}$ into this matrix of polynomials, 
which takes time
\[
\SoftOh(b^2 K (a \log(p)) (a d^n)) = \SoftOh(a^3 p d^{4n}).
\]
Computing $D(\hat{\tau})^{-1} \in \QQ_{\mathfrak{q}}$ and performing 
the scalar multiplication can be ignored.  Hence the computation 
takes time $\SoftOh(a^3 p d^{4n})$.
...
and $D(t) \Phi(t) \bmod{t^{K}}$ needs space
 \[
\SoftOh(b^2 K (a' \log(p)) (a d^n)) = \SoftOh(a^2 a' p d^{4n}),
\]
