...
\subsection{Evaluating of $\Phi$ at a point}
...
where $s$ divides some power of the polynomial $R$ from Definition~, such that 
...
Moreover, we may assume that $\theta_z=\theta_{z'}$ if $z,z'$ are conjugated over $\QQ_q$.
We now define
\begin{align*}
...
and $\theta_{\infty}=9$ at $\infty$. At the zeros $z$ of $r_1$, which are apparent singularities, 
we get a better bound by applying Theorem~\ref{thm:Gerkmann} with $\mu_z=-1$, $\nu_z=0$ and find 
$\theta_z=1$. After checking that, we can take $s=r_1 r_2^{660}$ and $K=31720$. We now compute the remaining 
