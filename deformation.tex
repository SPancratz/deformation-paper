...
Theorem~\ref{thm:Gerkmann} in order to find, for all $z \in D$, numbers $\theta_z \in \NN_0$ 
...
is restricted to the case $q = p$, i.e.\ the families of hypersurfaces 
...
$\mathfrak{q} \chi(T/\mathfrak{q})$ is the integer polynomial 
\begin{multline*}
...
computation of the $p$-adic precision parameter ``$\delta$'', which involves 
solving $b^2$~differential equations similar to the one for the matrix~$C$. 
...
not satisfy Theorem~\ref{thm:weildeligne}.
...
As the final result of the computation we obtain the 
integer polynomial~$\chi(T)$, 
\begin{multline*}
...
As the final result of the computation we obtain the polynomial~$7 \chi(T/7)$, 
\begin{multline*}
...
family of generic quartic surfaces, we were not able to compute the 
exponents at the zeros of $r_2$ and $r_3$ within reasonable time and thus 
refained from using them in the precision analysis.  For the family of 
generic quartic surfaces, this required $13s$ and $59s$ at the zeros of~$r_1$ 
and at~$\infty$, respectively, but almost four hours for both the zeros 
of~$r_2$ and $r_3$.  Note, however, that in this last case we only use the 
exponents at~$\infty$ in the precision analysis in order to guarantee that 
the computation is provably correct.
\end{rem}
