...
\begin{thm}
Frobenius structure on algebraic de Rham cohomology in liftable normal crossing case.
\end{thm}
...
$\{\mathcal{F}_k \HdR^{n}(\mathfrak{U}_L) \}_{k \geq 0}$ 
...
basis $\cB$ by
\[
r = \prod_{k=\lfloor n/d \rfloor+1}^{n+1} \bigl( \det(\Delta_k) \bigr),
\]
...
$G/r$, with $G \in M_{b \times b}(\QQ_q[t])$.
\end{thm}
...
We first describe an explicit formula for the $\dim \HdR^n(\mathfrak{U}_0)$ 
nonzero coefficients of~$\Phi_0$ due to Dwork~\citep{Dwork1964},
...
\eqref{eq:alphap.0} to pairs $m,r \geq 0$ such that $m \leq \mathcal{M}$, or 
...
\State \begin{compactenum}[\it {Step} I.] \vspace{-1.24em}
\item Determine $\tilde{N}$, $\mathcal{M}$ and $\mathcal{R}$. Precompute the Teichm\"uller lifts $\hat{a}_0, \dotsc, \hat{a}_n$, 
      and the sequences $(d^{-r})_{r=0}^{\mathcal{R}}$ and $(\mu_m)_{m=0}^{\mathcal{M}}$, 
...
\item Compute the expression $x_1 = (-1)^{k_u+k_v} (k_v-1)! p^n$ as an 
      exact integer [[TODO Seb, what is the right precision here?]].
...
Recall that $\Phi \in M_{b \times b} \bigl(\QQ_q \langle t,\frac{1}{r(t)} \rangle^{\dag} \bigr)$ denotes 
the matrix for the $\sigma$-semilinear action of~$p^{-1} \Frob_p$ on $\Hrig^{n}(U/S)$ with 
respect to the basis $\cB$, where $\sigma$ is the standard lift of the $p$th-power Frobenius 
sending $t \mapsto t^p$, and $r$ is defined as in Definition \ref{defn:resultant}. Also recall 
from Theorem \ref{thm:denom} that $M = G/r$, where $G \in M_{b \times b}(\QQ_q[t])$, denotes 
the matrix for the Gauss--Manin connection $\nabla$ on $\HdR^n(\mathfrak{U}/\mathfrak{S})$
with respect to the same basis. As we saw in section [[TODO Jan]], these matrices 
...
\State \begin{compactenum}[\it {Step} I.] \vspace{-1.24em}
\item Determine $N_{\Phi_0},N_C,N_{C^{-1}},\tilde{N}_C$, and $\tilde{N}_{C^{-1}}$
\item $M \gets \textsc{GMConnection}(P)$
\item $\Phi_0 \gets \textsc{DiagFrob}(P_0,N_{\Phi_0})$
\item Solve Equation \eqref{eq:01-GMDE-Homogenous} for $C$ and Equation \eqref{eq:01-GMDE-Dual} for $C^{-1}$,using a recursion like in Equation \eqref{eq:recursion}, computing with $p$-adic working precision $\tilde{N}_C$ and $\tilde{N}_{C^{-1}}$, respectively
...
The bounds that
Lauder ~\citep[\S 8.1]{Lauder2004a} and Gerkmann~\citep[\S 6]{Gerkmann2007} used
were very far from being sharp, which significantly slowed down their algorithms.
Recently, under a small additional assumption, Kedlaya and the second 
...
$-\delta$ and $\bigl( -2 \delta + (n-1) \bigr)$, since we might not know what they are exactly, 
possibly at the expense of a slightly weaker bound.
\end{proof}
...
$\epsilon$ is usually not known. In practice it is often still possible to avoid having to 
use the greater of the two precisions in Theorem~\ref{thm:N0} by
computing $\epsilon$ first.  Writing $\chi(T) = \sum_{i=0}^{b} \chi_i T^i$, we can recover 
$\chi_0, \dotsc, \chi_{\floor{b/2}+1}$ provided that
\begin{equation}
p^N > 2 \max\biggl\{\binom{b}{\floor{b/2}} \mathfrak{q}^{\floor{b/2} (n-1) / 2}, 
                   \binom{b}{\floor{b/2}+1} \mathfrak{q}^{(\floor{b/2}+1) (n-1)/2} \biggr\}.
\end{equation}
This allows us to determine $\epsilon$ from the two 
coefficients $\chi_{\ceil{b/2}-1}$ and $\chi_{\floor{b/2}+1}$, if 
these are nonzero, and we can then recover the remaining 
coefficients using the functional equation. [[TODO Seb, so what
is the recommended strategy?]]
%TODO I thought a bit about this, and think the best is to first compute
%with the precision from this remark, and only when this is not sufficient,
%so when \chi_{\ceil{b/2}-1} turns out to be zero and both epsilons give a 
%Weil polynomial, raise the precision according to the largest i< \ceil{b/2}-1
%such that \chi_i is nonzero. This is perhaps a bit annoying to write down, but
%is it at least the 'optimal strategy'? What about that trick from Kedlaya to
%almost completely get rid of the binomial coefficients? He definitely used more
%about Weil polynomials then just the functional equation. 
\end{rem}
...
\begin{algorithmic}
\vspace{1mm}
\Require $P \in \ZZ_q[t][x_0,\ldots,x_n]$, $\tau \in S(\mathbb{F}_{\mathfrak{q}})$.
\Ensure  The zeta function $Z(X_{\tau},T)$ of the fiber $X_{\tau}$ lying over $\tau$.
\Procedure{ZetaFunction}{$P,\tau$}
\State \begin{compactenum}[\it {Step} I.] \vspace{-1.24em}
\item $N \gets ..$, $K \gets ..$
\item $\Phi \gets \textsc{FrobSeriesExpansion}(N,K)$ 
\item
\item \textbf{return} $Z(X,T)$
\end{compactenum}
\EndProcedure
\end{algorithmic}
\end{algorithm}
