...
denote the projection onto the subspace $W_k \leq V_k$.  Then the map 
\begin{equation}
...
there is precisely one non-zero entry in each column and each row of $\Delta_k$.  
Namely, in column $(j, g) \in C_k$ and row $g x_j^{d-1} \in R_k$ there is the 
non-zero entry $d \alpha_j$, concluding the proof.
\end{proof}
