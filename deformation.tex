...
we have that $\partial_j P_{t_0} (x_0, \dotsc, x_n) = d a_j x_j^{d-1}$, there is 
...
dt \otimes \frac{- k x^u P_t \Omega}{P^{k+1}},
\end{equation}
...
\begin{equation}
dt \otimes \frac{- k x^u P_t \Omega}{P^{k+1}} \equiv 
...
\end{equation}
Since $\ord_p(\tilde{M}-M) \geq N_M$, we have that 
\[
...
at most $K$. We then compute the matrix $\Phi$ to $t$-adic precision $K$ and $p$-adic
...
\begin{align*}
\mbox{time: }  &\SoftOh\bigl(p a^2 d^{2n} (p + d^n) \log^2(d_t) \bigr), \\
\mbox{space: } &\SoftOh\bigl(p^2 a^2 d^{2n} \log^2(d_t)\bigr).
\end{align*}
\end{prop}
