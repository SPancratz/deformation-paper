...
\[
\SoftOh\bigl(K \max\{\deg(G),\deg(r)\} b^{\omega} (a'\log(p)) (a d^n)\bigr) = 
\SoftOh(a^2 a' p d^{n(\omega+3)}e^n d_t).
\] 
Applying $\sigma$ to an element of $\QQ_q$ to the required precision
takes time $\SoftOh\bigl((a' \log(p))^2(a d^n)\bigr)$. Therefore, 
$\sigma(G)$ and $\sigma(r)$ can be computed in time 
\[
\SoftOh\bigl(b^2 (de)^n d_t (a' \log(p))^2 (a d^n)\bigr) = 
\SoftOh(a a'^2 \log^2(p) d^{4n} e^n ).
\] 
...
\[
\SoftOh\bigl(b^{\omega} K \log(q) (a d^n)\bigr) = \SoftOh(a^2 a' p d^{n(\omega+2)}).
\]
