...
\State \begin{compactenum}[{ } 1.] \vspace{-1.24em}
...
\item \begin{algorithmic} 
      \For{$j=0$ \textbf{to} $n$}
        \State $Q_j \gets \sum_{g \in C_k^{(j)}} v_g^{(j)} g$ ($v_g^{(j)}$ is the entry in $v^{(j)}$ corresponding to $g \in C_k^{(j)}$)
      \EndFor
      \State $\gamma_k \gets Q-(Q_0 \partial P_0 + \dotsb + Q_n \partial_n P)$
      \end{algorithmic}
%For $j = 0, \dotsc, n$, set $Q_j = \sum_{g \in C_k^{(j)}} v_g^{(j)} g$,
...
\item \begin{algorithmic}
      \State Let $\Phi_0 \in M_{b \times b}(\QQ_p)$ be the zero matrix.
      \For{$x^u \in B$} 
        \State Determine the unique $x^v \in B$ such that $v_i = p (u_i + 1) - 1 \bmod{d}$.
        \State $x_1 \gets (-1)^{k_u+k_v} (k_v-1)! p^n$ as an exact integer %TODO? a sufficient $p$-adic precision here is N+ord_p((n-1)!)+n+\delta
        \State $x_2 \gets (k_u - 1)! \alpha_{u,v} \pmod{p^{\tilde{N}}}$ using Theorem \ref{thm:alpha2} or 
               Theorem \ref{thm:alphap}.
        \State Compute $x_2^{-1} \pmod{p^{N-n}}$.
        \State $(\Phi_0)_{u,v} \gets x_1 x_2^{-1} \pmod{p^N}$
      \EndFor 
      \end{algorithmic} 
\item \Return $\Phi_0$      
\end{compactenum}
...
\State $C_{i+1} \gets \frac{-1}{r_0 (i+1)} \biggl(\sum_j G_{i-j} C_j + \sum_j r_{i-j+1} (j C_j) \biggr) \pmod{p^{\tilde{N}_C}}$
\EndFor
\State $C \gets \sum_{i=0}^K C_i t^i \pmod{p^{N_C}}$
\end{algorithmic}
\item Solve Equation \eqref{eq:01-GMDE-Dual} for $C^{-1}$ to $t$-adic precision $\floor{K/p}$ and $p$-adic precision $N_{C^{-1}}$:
\begin{algorithmic}
\State $(C^{-1})^t_0 \gets I$
\For{$i=0$ \textbf{to} $\floor{K/p}-1$}
\State $(C^{-1})^t_{i+1} \gets  \frac{-1}{r_0 (i+1)} \biggl(\sum_j (-G_{i-j})^t C_j^t + \sum_j r_{i-j+1} (j C_j^t) \biggr) \pmod{p^{\tilde{N}_{C^{-1}}}}$
\EndFor 
\State $C^{-1} \gets \sum_{i=0}^{\floor{K/p}} (C^{-1})_i t^i \pmod{p^{N_{C^{-1}}}}$
\end{algorithmic}
...
\begin{algorithmic}
\vspace{1mm}
\Require $P \in \ZZ_q[t][x_0,\ldots,x_n]$ satisfying .., $\tau \in S(\mathbb{F}_{\mathfrak{q}})$.
\Ensure  The zeta function $Z(X_{\tau},T)$ of the fiber $X_{\tau}$ lying over $\tau$.
\Procedure{ZetaFunction}{$P,\tau$}
\State \begin{compactenum}[{ } 1.] \vspace{-1.24em}
\item \begin{algorithmic}
\If{$\epsilon$ is known} 
  \State $N \gets \ceil{\log_p ( 2 \binom{b}{\floor{b/2}} \mathfrak{q}^{\floor{b/2} (n-1) / 2})}$ 
\Else
  \State $N \gets \ceil{\log_p (2 \max_{0 \leq i \leq b} \{ \binom{b}{i} \mathfrak{q}^{i (n-1) / 2} \})}$ 
\EndIf
\State $\tilde{N} \gets N + \delta$
\end{algorithmic}
