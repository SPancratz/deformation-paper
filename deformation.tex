...
\abstract{In this paper we present various improvements to the deformation method
for computing the zeta function of smooth projective hypersurfaces over a finite 
field using $p$-adic cohomology. We give new bounds for the $p$-adic and $t$-adic 
precisions required to obtain provably correct results and show how the different 
steps of the method can be carried out in a more efficient way. The algorithm that we 
obtain this way has a lower time and space complexity than existing methods. Moreover,
our implementation is a lot more practical and can be applied more generally than
previous ones. We illustrate this with some examples that we have computed for 
curves and surfaces.}
