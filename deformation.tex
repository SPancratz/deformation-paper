...
\end{cases}
\end{equation}
using only one $p$-adic inversion [[TODO Seb: perhaps explain a bit better why only one inversion]].  
...
sending $t \mapsto t^p$. Also recall that $M \in M_{b \times b}(\QQ_q[t,\frac{1}{r(t)}])$ denotes 
...
[[TODO Jan]], we may suppose that $R_0 \neq 0 \pmod{p}$, so in particular $R_0 \neq 0$.
...
\begin{thm}
Let $N \in \ZZ$ and suppose that $N \geq \ord_p(\Phi_0)$ [[TODO Jan, try to get rid of this]]. In order to compute the power series expansion 
...
\begin{algorithmic}
\vspace{1mm}
\Require $t$-adic precision $K \geq 0$, $p$-adic precision~$N \geq 0$.
\Ensure  Power series expansion of the matrix $\Phi$ at $t=0$ to $t$-adic precision $K$ and $p$-adic precision $N$.
\Procedure{SeriesExpansion}{$N,K$} [[TODO finish, more inputs, ..]]
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
In this section we want to compute the zeta function of the fiber $X_{\tau}$ of
...
\begin{equation}
Z(X_{\tau},T) = \frac{p(T)^{(-1)^n}}{(1 - T) (1 - \mathfrak{q}T) \dotsm (1 - \mathfrak{q}^{n-1}T)},
\end{equation}
where $p(T) = \det \bigl( 1 - T \mathfrak{q}^{-1} \Frob_{\mathfrak{q}} | \Hrig^n(U_{\tau}) \bigr) \in \ZZ[T]$ 
...
of $p^{-1} \Frob_p$ on~$\Hrig^{n}(U_{\tau})$. As this action is $\sigma$-semilinear, we have that 
\begin{equation}
\Phi_{\tau}^{(a)} = 
...
\begin{proof}
Since~$\sigma$ preserves $p$-adic valuations, the valuations of the matrices 
$\Phi_{\tau}, \sigma(\Phi_{\tau}), \dotsc, \sigma^{a-1}(\Phi_{\tau})$ 
are all at least $-\delta$ by [[TODO Jan]]. So the theorem follows from
Proposition \ref{prop:matrixproductval}. 
\end{proof}

We now address the loss of $p$-adic precision when computing the reverse
characteristic polynomial $\det \bigl( 1 - T \mathfrak{q}^{-1} \Frob_{\mathfrak{q}} | \Hrig^n(U_{\tau}) \bigr)$.  
From the definition of the determinant, it appears that the precision loss could be as 
great as $(b-1) \ord_p(\Phi_{\tau}^{(a)})$.  However, because of the special properties of $\Phi_{\tau}^{(a)}$,
we have a much better bound:

\begin{thm}
Suppose that $\tilde{\Phi}_{\tau}^{(a)}$ is an approximation to 
$\Phi_{\tau}^{(a)}$ with 
\[
\ord_p\bigl(\Phi_{\tau}^{(a)}-\tilde{\Phi}_{\tau}^{(a)}\bigr) \geq N + \delta.
\]
Then we have
\begin{equation}
...
\begin{proof} 
See Gerkmann~\citep[Lemma~3.3, Lemma~3.4]{Gerkmann2007}. [[TODO Jan, check Gerkmann and include proof]]
\end{proof}
Finally, we ask to what $p$-adic precision~$N$ we need to compute
$\det\bigl(1 - T \Phi_{\tau}^{(a)} \bigr)$ in order to recover 
$p(T)$.

\begin{thm} \label{thm:N0}
In order to recover $p(T)$, it suffices to compute 
...
\end{equation}
provided that the sign $\epsilon = \sgn(\det(\Phi^{(a)}))$ is known.
\end{thm}
...
\begin{rem}
It is well known that $\epsilon = 1$ when $n$ is even.
\end{rem}

\begin{rem}
When $n$ is odd, $\epsilon$ is usually not known. However, in practice it is often still 
possible to avoid having to use the greater of the two precisions in Theorem~\ref{thm:N0} by
computing $\epsilon$ first.  Writing $p(T) = \sum_{i=0}^{b} a_i T^i$, we can recover 
$a_0, \dotsc, a_{\floor{b/2}+1}$ provided 
\begin{equation}
