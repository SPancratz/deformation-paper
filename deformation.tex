...
are well known bounds [[TODO]] for the degrees of the numerator and 
denominator of $Z(X,T)$, that reduce its computation to that 
...
For example, when $X$ is an elliptic curve, the Schoof-Atkins-Elkies 
algorithm [[TODO]], that uses $\ell$-adic \'etale cohomology, has runtime 
polynomial in $\log(q)$, and is also very fast in practice.
...
[[TODO]]. The same idea has been shown to work in much greater 
generality, for example for smooth projective hypersurfaces [[TODO]]. 

Lauder [[TODO]] showed that instead of computing the action of the Frobenius map on
...
Actually, this is not entirely correct, because in [[TODO]] Lauder did not work 
...
stated and proved in rigid cohomology. In [[TODO]], Gerkmann reformulated Lauder's
deformation algorithm in terms of rigid cohomology. Moreover, he improved various 
precision bounds to make the algorithm more practical, and computed a lot of examples.
...
Lauder's algorithm. Moreover, the first author has written a very fast publicly
available C implementation of the algorithm using the library FLINT [[TODO]]. 
This implementation lowers the runtime of the examples in [[TODO]] by at least
...
there are already good references available for the relevant theory [[TODO]]. 
