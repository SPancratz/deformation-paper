...
by Bombieri~\cite{Bombieri1966} for the degrees of the numerator and 
...
a lot of attention, resulting in very efficient algorithms.  For example, 
when $X$ is an elliptic curve, Schoof's algorithm \cite{Schoof1995}, which 
uses $\ell$-adic \'etale cohomology, has runtime polynomial in~$\log(q)$, 
and is also very fast in practice using some improvements by Atkins and 
Elkies.
...
hyperelliptic curves in odd characteristic~\cite{Kedlaya2001}.  The same 
idea has been shown to work in much greater generality, for example for 
smooth projective hypersurfaces \cite{AbbottKedlayaRoe2006}. 

Lauder \cite{Lauder2004a,Lauder2004b} showed that instead of computing the 
...
To be more precise, in~\cite{Lauder2004a,Lauder2004b} Lauder did not directly 
...
are more easily stated and proved in rigid cohomology.  In~\cite{Gerkmann2007} 
...
lowers the runtime of the examples in \cite{Gerkmann2007} by at least three 
orders of magnitude.

We now briefly describe the contents of the remaining sections.  We 
necessarily rely on some terminology that is introduced only in 
Section~\ref{sec:Background}.  The reader who is not familiar with this 
terminology might prefer to start reading there.
...
see e.g.~\cite{Kedlaya2012}. We also introduce the required terminology and 
...
for controlling $p$-adic precision loss in the algorithm.
