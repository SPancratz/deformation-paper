...
%or Lauder~\citep[\S 6.1]{Lauder2004b}.
...
In this section we determine the complexity of Algorithm \ref{alg:complete}. Recall that $a = \log_p(\mathfrak{q})$,
...
\begin{align*}
\mbox{time: }  &\tilde{\mathcal{O}}(a \log(p) \log(q) (\max \{ d^{n(\omega+2)} e^{n(\omega+1)}, d^{5n}e^{3n} \}) d_t), \\
\mbox{space: } &\tilde{\mathcal{O}}(a \log(p) \log(q) d^{4n}e^{3n} d_t).
\end{align*}
\end{prop}
...
\[
\tilde{\mathcal{O}}((de)^{n \omega} \log(q) (a \log(p) d^n) (de)^n d_t),
\] 
and there are $\mathcal{O}(n)$ of them, so this can be done in time 
$\tilde{\mathcal{O}}(a \log(p) \log(q) d^{n(\omega+2)} e^{n(\omega+1)} d_t)$.
...
\[
\tilde{\mathcal{O}}((de)^{2n} \log(q) (a \log(p) d^n) (de)^n d_t)=\tilde{\mathcal{O}}(a \log(p) \log(q)d^{4n}e^{3n} d_t),
\]
and there are $b=\mathcal{O}(d^n)$ of them, so this will take
time $\tilde{\mathcal{O}}(a \log(p) \log(q)d^{5n}e^{3n}d_t)$. 

During this computation we have to store $\mathcal{O}(n)$ matrices (and vectors) of size 
\[
\tilde{\mathcal{O}}((de)^{2n} \log(q) (a \log(p) d^n) (de)^n d_t),
\] 
...
\begin{align*}
\mbox{time: }  &\tilde{\mathcal{O}}(a^2 p \log(q) d^{n(\omega+3)}e^n d_t), \\
\mbox{space: } &\tilde{\mathcal{O}}(a^2 p \log(q) d^{4n}).
\end{align*}
\end{prop}
...
\[
\tilde{\mathcal{O}}(K \max\{\deg(G),\deg(r)\} b^{\omega} \log(q) (a \log(p) d^n)) = 
\tilde{\mathcal{O}}(a^2 p \log(q) d^{n(\omega+3)}e^n d_t).
\] 
Applying $\sigma_p$ to an element of $\QQ_q$ to the required precision
takes time $\tilde{\mathcal{O}}(\log^2(q)(a\log(p)d^n))$. Therefore, $\sigma_p(G)$ and $\sigma_p(r)$ 
can be computed in time 
\[
\tilde{\mathcal{O}}(b^2 (de)^n d_t \log^2(q) (a\log(p)d^n))=\tilde{\mathcal{O}}(a \log(p) \log(q)^2 d^{4n} e^n ).
\] 
Now the computation of the matrix $\sigma(C)^{-1}$ takes time $\tilde{\mathcal{O}}(a^2 \log(q) d^{n(\omega+3)}e^n d_t)$. 
Finally, the matrix product $C \Phi_0 \sigma_p(C)^{-1}$ can be computed in time 
\[
\tilde{\mathcal{O}}(b^{\omega} K \log(q) (a \log(p) d^n)) = \tilde{\mathcal{O}}(a^2 p \log(q) d^{n(\omega+2)}).
\]
...
\[
\tilde{\mathcal{O}}(b^2 (a \log(p))(a \log(p) d^n)) = \tilde{\mathcal{O}}(a^2 \log(p) \log(q) d^{3n}),
\]
and $D(t) \Phi(t) \bmod{t^{K}}$ needs space
 \[
\tilde{\mathcal{O}}(b^2 K \log(q) (a \log(p) d^n))= \tilde{\mathcal{O}}(a^2 p \log(q) d^{4n}),
\]
so that the total space required is $\tilde{\mathcal{O}}(a^2 p \log(q) d^{4n})$.
\end{proof}
...
\begin{align*}
\mbox{time: }  &\tilde{\mathcal{O}}(a^3 p d^{4n} + a^2 p \log(q) d^{n(w+3)} e^n d_t+ a \log(q) (\max \{ d^{n(\omega+2)} e^{n(\omega+1)}, d^{5n}e^{3n} \}) d_t), \\
\mbox{space: } &\tilde{\mathcal{O}}(a^2 p \log(q) d^{4n}+a \log(q) d^{4n} e^{3n} d_t).
\end{align*}
\end{thm}
...
\begin{rem}
When $\mathfrak{q}=q$ and $d_t=1$ as in [[TODO]], the bounds for the time and space complexity simplify to
\begin{align*}
...
The time complexity of Algorithm~\ref{alg:Diagfrob} is given by 
\begin{equation*}
...
\begin{proof}
We consider each of the steps in Algorithm~\ref{alg:Diagfrob}. 
