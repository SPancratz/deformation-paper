...
computed efficiently. Here efficiently could mean with low time complexity or 
just fast in practice. When $X$ is a (hyper)elliptic curve, this problem
is important for cryptographic applications, and has therefore received
...
improvements to every step of the algorithm. 
...
This implementation lowers the runtime of the examples in \cite{Gerkmann2007} by at least
three orders of magnitude. So our work is a significant improvement of previous work.
...
the Gauss--Manin connection. We prove some lower bounds for the valuation of 
...
matrix at some fibre, and deduce its zeta function. We combine bounds and tricks 
from various sources to bring down the required $p$-adic and $t$-adic precisions 
as much as possible. This results in Algorithm \ref{alg:complete}, which combines
all previous algorithms, and is the main result of this paper.
...
the $p$-adic valuation on $\QQ_q$ by $\ord_p$, and extend it 
to polynomials and matrices over $\QQ_q$ in the usual way, i.e. 
by taking the minimum over the coefficients and entries, 
respectively.
...
and the point at infinity, for some $\rho<1$. We extend the $p$-adic valuation 
to this ring, and polynomials and matrices over it, by defining
\[
\ord_p \Bigl( \sum_{i,j=0}^{\infty} a_{i,j} \frac{t^i}{r(t)^j} \Bigr) = \min_{i,j} \{ \ord_p(a_{i,j})\}.
\]
\end{defn}
...
$M_{b \times b}(\QQ_q \langle t, \frac{1}{r(t)} \rangle^{\dag})$ 
...
$[w_1,\ldots,w_b]$ we only have that $\ord_p{\Phi'} \geq -\delta'$. 
\end{rem}
