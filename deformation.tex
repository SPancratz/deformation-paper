...
improvements to just about every step of the algorithm. 
...
We now briefly describe the contents of the remaining sections in the paper. We
will use some terminology that is introduced only in Section \ref{sec:Background}.
The reader that is not familiar with the terminology yet should start reading there. 
...
the end of this section we give an overview of the deformation method.
...
throughout the paper. Our most important theoretical result in this section
is Theorem \ref{thm:Isomorphism}, which allows us to compute more efficiently 
in the cohomology, using the method of Griffiths and Dwork. The matrix of the 
Gauss--Manin connection is computed in Algorithm \ref{alg:Connection}. At the
end of this section we give lower bounds for the valuation of the matrix of 
Frobenius, that are essential to control $p$-adic precision loss.

In Section \ref{sec:Diagonal}, \\

In Section \ref{sec:DifferentialSystem}, \\

In Section \ref{sec:ZetaFunctions}, \\
