...
For any finite field extension $\FF_{\mathfrak{q}}$ of $\FF_q$ and
$\tau \in S(\FF_{\mathfrak{q}})$,
\[
...
we let $\FF_{\mathfrak{q}}$ be a finite field extension of $\FF_q$ and 
...
$\HdR^n(\mathfrak{U}_{\QQ_q(t)})$, as announced before.  We begin with an 
...
H_j &= \{(u_0,\dotsc,u_n) \in (\ZZ/d\ZZ)^{n+1} : u_j = -1 \},
\end{align*}
...
$\HdR^n(\mathfrak{U}_{\QQ_q(t)})$. However, we know from Theorem~\ref{thm:hypersurface} that 
\[
\dim \HdR^n(\mathfrak{U}_{\QQ_q(t)}) = \bigl((d-1)/d\bigr) \bigl( (d-1)^n - (-1)^n \bigr).
\]  
...
a basis for $\HdR^n(\mathfrak{U}/\mathfrak{S})$: it 
spans the cohomology since one can apply Algorithm~\ref{alg:PoleRed}, and it is 
linearly independent because it is so generically. Therefore, it will be convenient
to choose $\mathcal{S}$ a bit smaller, so that this condition is satisfied. 
\end{rem}

\begin{assump} \label{assump:R}
From now on we will always assume that the zero locus of $R$ in 
$\mathbf{P}^1_{\ZZ_q}$ does not intersect $\mathcal{S}$, so that 
$\cB$ is a basis for $\HdR^n(\mathfrak{U}/\mathfrak{S})$.
\end{assump}
...
-\delta \leq \ord_p\bigl((k(v)-1)!\bigr) + n 
           - \ord_p\bigl((k(u)-1)! \alpha_{u,v}\bigr)
\end{equation*}
Since $k(v) \leq n$, the result follows.
\end{proof}
...
Algorithm~\ref{alg:Diagfrob} below.
...
\end{equation}
Since $\ord_p(\tilde{M}-M) \geq N_M$, we have 
\[
\ord_p \Bigl( \overline{\int_{0}^t \bigl( \tilde{M}(s)-M(s) \bigr) ds }\Bigr) \geq 
N_C + (2 \delta + n-1) \ceil{\log_p(K-1)}+1,
\]
so for all $1 \leq i \leq K-1$, we obtain the bound
\begin{equation*}
\ord_p \Bigl( \overline{\frac{1}{i!} \Bigl(\int_{0}^t \bigl( \tilde{M}(s)-M(s) \bigr) ds \Bigr)^i } \Bigr) \geq 
N_C + (2 \delta + n-1) \ceil{\log_p(K-1)},
\end{equation*}
where we have used that $\ord_p(i!) \leq i$. Moreover, from Theorem~\ref{thm:valC}, we already know that 
\[
...
Now we explain how to compute the solution to Equation~\eqref{eq:01-GMDE-Homogenous}. We can 
write $M = G/r$, with $G = \sum_{i=0}^{\deg(G)} G_i t^i \in M_{b \times b}(\QQ_q[t])$, 
...
our computations. By Assumption~\ref{assump:diag}, we have
$r(0) \neq 0 \pmod{p}$, so in particular $r(0) \neq 0$.  

We can now obtain a power series solution $C = \sum_{i=0}^{\infty} C_i t^i$ to
the equation
\begin{align*}
...
\end{align}
We will only carry out this computation to some finite $p$-adic 
...
using Theorem~\ref{thm:valC}, as we will now show. 
...
One now checks that the matrix $C^{-1}\tilde{C}$ satisfies 
the differential equation
\begin{align*}
...
series expansion for $\Phi$ at $t=0$ to any given $p$-adic and $t$-adic precisions.
...
lift~$\hat{\tau}$ of some nonzero $\tau \in S(\FF_{\mathfrak{q}})$, but 
...
that our basis vectors for $\Hrig(U/S)$ %TODO, not really Hrig(U/S), only after shrinking S
...
For any $N_{\Phi} \in \NN$, 
we can use Theorem~\ref{thm:KedlayaTuitman} or Theorem~\ref{thm:Gerkmann}
to find $K \in \NN$, and $D \in \ZZ_q[t]$ that divides some power of $r$, such that 
$D \Phi$ is congruent modulo $p^{N_{\Phi}}$ to a matrix of polynomials of degree 
less than $K$. We then compute the matrix $\Phi$ to $t$-adic precision $K$ and $p$-adic
precision $N_{\Phi}$ using Algorithm~\ref{alg:expansion}. For any $\tau \in S(\bar{\FF}_q)$, 
we can now compute
\begin{equation*}
...
$\ord_p(\Phi) \geq -\delta$, 
the matrix $\Phi_{\tau}$ will then also be correct to precision $N_{\Phi}$ 
...
from Section~\ref{sec:Connection} as follows.
...
\]
Let $N_{\Phi} \in \NN$ be such that $N_{\Phi} \geq \Gamma(b)$.
Suppose that $\tilde{\Phi}_{\tau}$ is an approximation to $\Phi_{\tau}$ 
satisfying $\ord_p (\Phi_{\tau}-\tilde{\Phi}_{\tau}) \geq N_{\Phi}$. 
Then for all $1 \leq i \leq b$, we have
\[
\ord_p(\chi_i-\tilde{\chi}_i) \geq N_{\Phi} + \Gamma(i-1).
\]
\end{thm}
...
N_{\Phi} &= \max_{1 \leq i \leq b} \{ N_{\chi_i} -\Gamma(i-1) \}.
\end{align*}
\end{thm}
...
\item Determine $K \in \NN$, and $D \in \ZZ_q[t]$ that divides some power of $r$, such that $D \Phi$ is a matrix 
      of polynomials of degree at most $K$ to $p$-adic precision $N_{\Phi}$, using Theorem~\ref{thm:KedlayaTuitman}
      or Theorem~\ref{thm:Gerkmann}.
