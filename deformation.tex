...
into $\Phi$,
to obtain the matrix $\Phi_{\tau}$ of the action of $p^{-1}\Frob_p$ on $\Hrig^n(U_{\tau})$. 
Compute the matrix $\Phi_{\tau}^{(a)}$ of the action of 
$\mathfrak{q}^{-1} \Frob_{\mathfrak{q}}$, which is also equal 
to $(p^{-1}\Frob_p)^a$, on~$\Hrig^n(U_{\tau})$. Use 
...
In order to compute $(k_u-1)!\alpha_{u,v}$ to $p$-adic precision $\tilde{N}$, it 
suffices to restrict the sums in Theorem \ref{thm:alpha2} or Theorem \ref{thm:alphap} 
to pairs $m,r \geq 0$ such that $m \leq \mathcal{M}$, or equivalently $r \leq \mathcal{R}$, 
where 
\begin{eqnarray*}
\mathcal{M} &=& \floor{\frac{p^2}{p-1}(\tilde{N}+\log_p(\tilde{N}+4)+5)}, \\
\mathcal{R} &=& \floor{\mathcal{M}/p}.
\end{eqnarray*}
\end{prop}

\begin{proof}
We know from the proofs of Theorem \ref{thm:alpha2} and Theorem \ref{thm:alphap} that the terms in the 
sums over $m,r$ are $p$-adically integral. Moreover, the valuations of the terms
with $r>0$ satisfy
\begin{align*}
\ord_p \biggl( \Bigl( \frac{u_i+1}{d} \Bigr)_r \biggl( \sum_{k=0}^{\floor{m/p}} \frac{p^{r-k}}{(m-pk)!k!} \biggr) \biggr) &\geq
\frac{r}{p-1}-\floor{\log_p(r)+1} + \frac{p-1}{p^2} m + \floor{m/p}-\frac{m}{p-1} \\
&\geq \Bigl( \frac{p-1}{p^2} \Bigr) m - \log_p(m) - 2,
\end{align*}
by the bound \eqref{eqn:dworkbound}, Lemma \ref{lem:lambdam}, Proposition \ref{prop:mpr}, and Proposition \ref{prop:rfac}. 
Therefore, it is sufficient to restrict the sums in Theorem \ref{thm:alpha2} or Theorem \ref{thm:alphap} to $m,r \geq 0$ for which 
\begin{equation*}
\Bigl( \frac{p-1}{p^2} \Bigr) m - \log_p(m) - 2 < \tilde{N}.
\end{equation*}
...
\begin{rem}
The condition that $M$ has a simple pole at $z$ is not a serious restriction, since
...
to find $K \in \NN$, and a polynomial $D \in \ZZ_q[t]$ not divisible by $p$, the zeros of which are zeros of $r$, such that 
...
by Corollary~\ref{cor:delta}, it is sufficient to compute $\hat{\tau}$ with $p$-adic precision $N+\delta$.
...
Order the elements of $\cB = \bigcup_{k=1}^n \cB_k$ by decreasing $k$, and let $k(i)$ denote
...
\item[] \hspace{1em} $\tilde{N} \gets N$, $\overline{N} \gets N$ 
\item[] \textbf{else}
\item[] \hspace{1em} $\tilde{N} \gets N + \delta$, $\overline{N} \gets N+(a+b-2) \delta$
...
\item $\chi(T) \gets \det\bigl(1-T \bigl(\Phi_{\tau} \sigma(\Phi_{\tau}) \ldots \sigma^{a-1}(\Phi_{\tau}) \bigr)  \bigr) \pmod{p^{N}}$, using $p$-adic working precision $\overline{N}$
...
$\tilde{\mathcal{O}}(a\log(p)d^n)$ by Theorem~\ref{thm:Ni} and 
Corollary~\ref{cor:Ntilde}.
\end{proof} 
