...
Finally, one uses the excision short exact sequence
...
proof of \eqref{eq:formulab} on the other hand can be found in [[TODO Jan, hard to find]]. 
\end{proof}
...
\begin{proof}
The only time in Algorithm \ref{alg:Connection} that nonconstant denominators are introduced is when the subroutine {\sc{Reduce}} calls its subroutine {\sc{Decompose}} and a linear system $\Delta_k v = b$ is solved. Since this happens only for $k=\ceil{(n+1)/d}, \dotsc, n+1$ and at most once for each such $k$, the result is clear.
\end{proof}
...
matrix $M$. Recall that $\FF_{\mathfrak{q}}$ denotes a finite extension of $\FF_q$. 
...
%Moreover, these summands are of size $\BigOh(m \log m)$ bits, which also allows us 
%to avoid performing intermediate reductions modulo~$p^{\tilde{N}}$.
%TODO doesn't make sense to me that O(m log m) would imply no reductions, effective constant?
\end{rem}
...
\end{equation*}
From Corollary \ref{cor:delta}, we know that $\ord_p(\Phi) \geq -\delta$ and 
$\ord_p(\Phi^{-1}) \geq -\delta-(n-1)$.
\end{proof}
...
D_{i+1} &= \frac{-1}{r_0 (i+1)} \biggl(
    ...
    p^{\tilde{N}} E_{i+1},
\end{align*}
where $(E_i)_{i \geq 1}$ is a sequence of $p$-adically integral matrices i.e., such that the matrices $D_i$
...
numbers. Let $\Phi$ denote the matrix of the action of $p^{-1} \Frob_p$ 
...
\end{equation*}
Since $Q(\hat{\tau}) \in \ZZ_q^{\times}$ by Assumption [[TODO]], and $\ord_p(\Phi(t))) \geq -\delta$
...
where $\FF_{\mathfrak{q}}$ denotes a finite extension of $\FF_q$.  
...
%\begin{rem}
%If $n \geq 3$, or $\mathfrak{q} \geq 4$, or $k \geq 7$, then
%\[
%\max_{1 \leq i \leq k} \{ \bigl( \frac{b}{i} \bigr) \mathfrak{q}^{i (n-1) / 2} \} = 
%\bigl( \frac{b}{k} \bigr) \mathfrak{q}^{k (n-1) / 2}.
%\]
%\end{rem}
...
\item $\Phi_{\tau} \gets D(\hat{\tau})^{-1} \bigl( D(t) \Phi(t) \bmod{t^{K}} \bigr)|_{t=\hat{\tau}} \pmod{p^{N}}$ with working precision $p^{\tilde{N}}$
\item $\chi(T) \gets \det\bigl(1-T \bigl(\Phi_{\tau} \sigma_p(\Phi_{\tau}) \ldots \sigma_p^{a-1}(\Phi_{\tau}) \bigr)  \bigr) \pmod{p^{N}}$
