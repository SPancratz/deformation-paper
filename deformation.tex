...
\begin{rem} 
Note that in Algorithm~\ref{alg:Connection}, it is sufficient to invert 
the matrices~$\Delta_k^{-1}$ just once. After this, every call to {\sc{Decompose}} 
reduces to a multiplication by one of the~$\Delta_k^{-1}$. Actually, instead 
of computing the inverse $\Delta_k^{-1}$, it is better to compute the 
decomposition $\Delta_k = L U P$, where $L$ is lower triangular, $U$ is 
upper triangular and $P$ is a permutation matrix, which 
is guaranteed to exist for any square matrix. Since the matrices $\Delta_k$
contain a lot of zeros, methods from sparse linear algebra can be used to compute this 
decomposition. After this, every call to {\sc{Decompose}} reduces to solving a 
lower and an upper diagonal linear system. Although all of this 
does not improve the complexity of Algorithm~\ref{alg:Connection}, it does result 
in an enormous improvement in practice.
\end{rem}
...
%TODO small problem, if M is computed to some p-adic precision, the poles are not determined exactly,
%so R and r etc. should be computed to infinite precision?? 
...
Let $z \in \mathbf{P}^1(\bar{\QQ}_q)$ be a geometric point such that:
%such that $\mathfrak{Z}$ does not contain any other points 
%with the same reduction modulo~$p$.
\begin{enumerate} 
\item $M$ has at most a simple pole at~$z$,
\item $M$ has no other poles with the same reduction modulo~$p$ as $z$, 
\item the exponents $\lambda_1, \dotsc, \lambda_{b}$ of $M$ at $z$, which are defined 
      as the eigenvalues  of the residue matrix $(t - z) M \vert_{t=z}$ and 
      known to be rational numbers, are contained in $\ZZ_p \cap \QQ$. 
\end{enumerate}
Let $\delta$ be defined as in 
...
hypersurfaces $\mathcal{X}/\mathcal{S}$ is defined by the homogeneous polynomial 
$P \in \ZZ_q[t][x_0,\dotsc,x_n]$.

\begin{thm} \label{thm:Gerkmann}
Let $z \in \mathbf{P}^1(\bar{\QQ}_q)$ be a geometric point.
...
$K \in \NN$, where 
$D$ divides some power of the polynomial $R$ 
from Definition~\ref{defn:resultant}, such that
$D \Phi$ is congruent modulo 
$p^{N_{\Phi}}$ to a matrix of polynomials of degree less than $K$.
\end{thm}

\begin{proof}
For all geometric points
$z \in \mathbf{P}^1(\bar{\QQ}_q)$, using either Theorem~\ref{thm:KedlayaTuitman} 
or Theorem~\ref{thm:Gerkmann}, we can explicitly find $\theta_{z} \in \NN_0$ 
such that $\Phi$ is congruent modulo $p^{N_{\Phi}}$ to a matrix 
$\tilde{\Phi} \in M_{b \times b}(\QQ_q(t))$ for which
$\ord_z(\tilde{\Phi}) \geq -\theta_z$. By Theorem~\ref{thm:Gerkmann}, 
we can take $\theta_z=0$ for $z \neq \infty$ if $R(z) \neq 0$,
so that $\theta_z$ vanishes for all but finitely many $z$.
Moreover, by Theorem~\ref{thm:KedlayaTuitman}, we can also take
$\theta_z=0$ if the connection matrix $M$ 
does not have a pole with the same reduction modulo~$p$ as $z$. 
%[[TODO]], perhaps at some point make a remark about the theta_z being
%invariant under Gal(\bar{Qq}/Qq) which shows that D is defined over
%Qq, but this is a bit annoying to write down, perhaps leave it this way
We now define 
\begin{align*}
...
\begin{equation*}
\Phi_{\tau} = 
  D(\hat{\tau})^{-1} \bigl( D \Phi \bmod{t^{K}} \bigr)|_{t=\hat{\tau}} 
  \bmod{p^{N_{\Phi}}}.
\end{equation*}
...
and $\deg(D), K$ are both $\SoftOh(apd^n (de)^n d_t)$.
\end{prop}
...
\begin{align*}
\deg{D} &\leq \deg  \Bigl( \prod_{k=2}^n \det(\Delta_k) \Bigr) + (p(n+h(N_{\Phi}))-n) \deg(\det(\Delta_{n+1})), \\
K &\leq \deg(D)+ 1+\deg \Bigl(\prod_{k=2}^n \Delta_k^{-1}\Bigr) + (p(n+h(N_{\Phi}))-n) \deg(\Delta_{n+1}^{-1}) + ph(N_{\Phi}) d_t. \\
\end{align*} 
Consequently, we find that 
\[
\deg(D), K \in \SoftOh(apd^n (de)^n d_t).
\]
...
zeros~$z$ of $r$ and $\theta_{\infty}=12$ so that $K=1613$ for the $t$-adic 
precision. We now compute the remaining $p$-adic precisions
...
instead. At each of the zeros of~$r$ we find $\mu=0$, $\nu=-1$, $\theta_z=397$ 
and at $\infty$ we find $\mu=-3$, $\nu=-2$, $\theta_{\infty}=793$, so that 
$K=2779$ for the $t$-adic precision. We now compute the remaining $p$-adic 
precisions
...
zeros~$z$ of $r$ and $\theta_{\infty}=25$, so that $K=1146$ for the $t$-adic 
precision. We now compute the remaining $p$-adic precisions
...
$M$             &      & ?     & 0.00s & ?       &      & ?     \\
$\Phi_0$        &      & 2.65m & 0.04s & 5.86m   &      & 1.31m \\
$\Phi$          &      & 1.33m & 1.77s & 1.38m   &      & 0.89m \\
$Z(X_{\tau},T)$ &      & 3.96m & 7.40s & 101.40m &      & 1.09m \\
Total           &      & 7.94m & 9.21s & 108.64m &      & 3.29m \\ \bottomrule
\end{tabular}
\end{center}
