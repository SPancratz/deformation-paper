...
\begin{rem}
Note that $B_k = \emptyset$ when $k < (n+1)/d$ or $k \geq n+1$.
\end{rem}
...
$N-n$, because of the loss of precision in computing their inverses. Note 
that if a $p$-adic number $x$ is known to precision~$k$, then its inverse 
is in general only known to precision $k-2\ord_p(x)$. Therefore, we need an 
upper bound on the $p$-adic valuation of the elements $(k_u-1)!\alpha_{u,v}$.
...
\Bigl(\frac{d}{dt} + M\Bigr) \Phi = p t^{p-1} \Phi \sigma(M) 
\end{equation}
with initial condition $\Phi(0) = \Phi_0$, 
...
\end{equation}
with initial condition $C(0) = I$, where $I$ denotes the identity matrix, 
...
r \frac{dC}{dt} + G C = 0
\end{equation*}
with initial condition $C(0) = I$, which is clearly equivalent to 
Equation~\eqref{eq:01-GMDE-Homogenous}, using the recursion $C_0 = I$, 
\begin{equation} \label{eq:recursiondifeq}
...
\end{equation}
with initial condition $C^{-1}(0) = I$ that carries a Frobenius structure 
...
\begin{rem}
A result similar to Theorem \ref{thm:Gerkmann} was obtained by Gerkmann in \citep{Gerkmann2007}.
\end{rem}
...
\begin{align*}
\max_{1 \leq i \leq b} \{N_i\} \in \BigOh\bigl(a n b + \log(b) \bigr) 
...
\begin{proof}
Let $\tilde{N}_{\Phi_0} \in \SoftOh(a d^n \log(d_t))$ denote the $p$-adic 
working precision in Algorithm~\ref{alg:Diagfrob}.  We first consider the 
...
$\BigOh( \min \{ 1,(n/p) \} \mathcal{M} ) \subset \SoftOh(\tilde{N}_{\Phi_0})$ 
of them.  Every $\mu_m$ is a sum of $\SoftOh(\tilde{N}_{\Phi_0})$ terms, each 
of which can be computed from the previous one in $\BigOh(p)$ operations in 
$\ZZ/p^{\tilde{N}_{\Phi_0}}\ZZ$.  Therefore, the required subsequence can be 
computed in time $\SoftOh(\tilde{N}_{\Phi_0}^3 p)$.
...
$\SoftOh(\tilde{N}_{\Phi_0}^2 \log (p)) \subset \SoftOh(a^2 \log(p) d^{2n} \log^2(d_t))$.
\end{proof}
