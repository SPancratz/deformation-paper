...
and the fact that $\ord_p(\Phi_0) \geq -\delta$, to obtain the sufficient $p$-adic precisions for the matrices 
...
on $\Hrig^{n}(U/S)$, with respect to the basis $[v_1,\dotsc,v_b]$, and denote
$w=\ord_p(\Phi)+\ord_p(\Phi^{-1})$. For $i \in \NN$, put
\begin{align*}
f(i)=\max \{w \lceil \log_p(i) \rceil, 
(b-1) \ord_p(M) + w \lfloor \log_p(i) \rfloor \},
\end{align*}
and define 
\begin{align*}
c = \begin{cases}
0 & \mbox{if $\ord_p(M) \geq 0$} \\
\min\{0, i + f(i): i \in \NN\} & \mbox{if $\ord_p(N) < 0$}.
\end{cases}
\end{align*}
For $N \in \NN$, put
\begin{align*}
g(N) &= \max \{i \in \NN \; | \; i+ \ord_p(\Phi) + c + f(i)  < N \},
\end{align*}
and define
\begin{align*}
...
than or equal to $-(\alpha_1+p \alpha_2)$ at $z$ (that is, the entries of the difference between 
the two matrices all have $p$-adic valuation at least $N$). 
\end{thm}

\begin{proof}
Since the matrix $\Phi$ defines a Frobenius structure on the vector bundle $\Hrig^n(U/S)$ with
connection $\nabla$, this is \citep[Theorem~2.1]{KedlayaTuitman2012}. 
\end{proof}

\begin{rem}
In practice the constants $w,c$ and $\alpha_1$ are always very small, so that $g(N)$ is 
about $N$, and the lower bound for the order of $\Phi$ modulo $p^N$ at $z$ is roughly
$-pN$.
\end{rem}

\begin{rem}
Note that if $\ord_p(\Phi)$ or $\ord_p(\Phi)$ are not known, then we can
use the lower bounds [[TODO Jan]] for these instead, at the expense of a slightly
worse bound.
\end{rem}
...
implied already by the existence of a Frobenius structure [[TODO]].
...
If we want to compute the matrix $\Phi_{\tau}$ of the action of
$p^{-1} \Frob_p$ on $\Hrig^{n}(U_{\tau})$ to $p$-adic precision $N$, then we
can use Theorem \ref{thm:KedlayaTuitman} to find $m,K \in \NN$ such that
$r(t)^m \Phi(t)$ is congruent modulo $p^N$ to a matrix of polynomials of degree
at most $K$. We then compute
\begin{equation}
