...
in $\SoftOh(k^{\omega})$ ring operations.  It is known that 
$\omega \leq 2.3727$ and, moreover, that the inversion of an invertible 
$k \times k$ matrix has the same complexity.  We now turn to two 
specific $p$-adic operatsons.  Firstly, images of elements of 
$\QQ_{\mathfrak{q}}$ under powers of the $p$-th power operator~$\sigma^k$, 
for $0 < k < a$, can be computed to precision~$N$ in time 
$\SoftOh(a \log^2 p + a N \log p)$.  Secondly, the Teichm\"uller lift 
of an element of $\ZZ_{\mathfrak{q}} \bmod p$ can be computed to 
precision~$N$ in time $\SoftOh(a N \log^2 p)$.
...
are also all $\SoftOh(a d^n)$ by Corollary~\ref{cor:Ntilde} and 
Theorem~\ref{thm:Ni}, noting that the logarithms that appear 
in the theorem are to base~$p$.
\end{proof}
...
\begin{align*}
\mbox{time: }  &\SoftOh\bigl(a^2 a' p d^{n(\omega+2)}\bigr), \\ 
...
matrix product $D(t) \Phi(t) \bmod t^K$ over~$\QQ_q[t]$ 
requires time 
\begin{equation*}
\SoftOh\bigl( b^{\omega} K a' (a d^n) \log p \bigr) 
    \subset \SoftOh\bigl( a^2 a' p d^{n(\omega+2)} \bigr).
