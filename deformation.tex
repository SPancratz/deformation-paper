...
\blfootnote{Communicated by Felipe Cucker. Mathematics Subject Classification: 11Y99, 11M38. Keywords: finite fields, computing zeta functions, deformation method.}
...
suggestions on earlier versions of this paper. Finally, we thank the anonymous
...
Let $u, v \in \ZZ^{n+1}$ be such that we have
$x^u, x^v \in B$ and 
...
pole at $z \neq \infty$, and has order at least $-i p \deg_t(P)$ at $z=\infty$. 
...
using our implementation\footnotemark.  This implementation 
...
precisions in the case $p=2^9+9$. We first compute $\delta=0$ and $N_{\Phi}=N'_{\Phi}=3$. We find
...
$p$             & $2^2+1$ & $2^3+3$ & $2^4+1$ & $2^5+5$ & $2^6+3$ 	\\ \midrule
Computation     & 	  &         &         &         &               \\ \midrule  
$M$             & 0.10s   & 0.10s   & 0.10s   & 0.10s  	& 0.10s	  	\\
$\Phi_0$        & 0.01s   & 0.00s   & 0.01s   & 0.01s 	& 0.00s	  	\\
$\Phi$          & 5.89s   & 9.19s   & 9.38s   & 20.06s 	& 42.18s       	\\
$Z(X_{\tau},T)$ & 0.07s   & 0.14s   & 0.08s   & 0.37s 	& 0.91s	  	\\
Total           & 6.07s   & 9.43s   & 9.57s   & 20.54s 	& 43.19s      	\\ \bottomrule
\end{tabular} 
\end{center}

\begin{center}
\begin{tabular}{l l l l l l} \toprule
$p$             & $2^7 + 3$ & $ 2^8+1$	& $2^9+9$  	& $2^{10}+7$ 	& $2^{11}+5$	\\ \midrule
Computation     & 	    &          	&          	&            	& \\ \midrule  
$M$             & 0.10s     & 0.10s	& 0.10s    	& 0.10s	 	& 0.10s \\
$\Phi_0$        & 0.00s     & 0.00s	& 0.00s       	& 0.00s	      	& 0.00s\\
$\Phi$          & 84.78s    & 166.68s	& 359.06s    	& 748.57s	& 1569.01s\\
$Z(X_{\tau},T)$ & 1.61s     & 3.62s	& 7.09s      	& 14.50s	& 32.03s\\
Total           & 86.49s    & 170.40s	& 366.25s    	& 763.18s	& 1601.15s\\ \bottomrule
\end{tabular} 
\end{center}

\bigskip

Our timings in this example confirm that the running time of the algorithm is (quasi)linear in $p$
and the memory usage also turns out to be (quasi)linear in $p$ as predicted. In the case $p=2^{11}+5$, the computation 
requires about 6 GB of memory. As an example, for the prime $p=2^{11}+5$ we find that $p \chi(T/p)$ is equal to
\begin{multline*}
2053 T^{21} - 4885 T^{20} + 6922 T^{19} - 7050 T^{18} + 1163 T^{17} + 7077 T^{16} - 12948 T^{15} \\ 
+ 12948 T^{14} - 9130 T^{13} + 3722 T^{12} + 128 T^{11} + 128 T^{10} + 3722 T^9 - 9130 T^8 \\ 
+ 12948 T^7 - 12948 T^6 + 7077 T^5 + 1163 T^4 - 7050 T^3 + 6922 T^2 - 4885 T + 2053.
\end{multline*}
