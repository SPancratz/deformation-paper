...
polynomials $\chi(T)$ that we we obtained from these computations. \\ 
...
\begin{rem}
We should mention some problems with Gerkmann's examples and timings. First, he does not 
include timings for part of his computations. For example, he does not mention the time 
it takes to compute the $p$-adic precision parameter that he calls $\delta$ (which involves 
solving $b^2$ differential equations similar to the one for the matrix $C$!). Also, there 
seem to be some mistakes in his precision analysis. For example, for the example in this section, 
his final $p$-adic precision is not high enough to recover the exact zeta function with 
the precision bounds that he is using. Finally, the polynomial $\chi(T)$ that he gives for 
the example in this section does not satisfy Theorem~\ref{thm:weildeligne} and therefore 
must be wrong.
\end{rem}
...
at the zeros of $r_1$ and at $\infty$ it took $13s$ and $59s$, 
respectively, but for both the zeros of $r_2$ and $r_3$ it took almost
$4$ hours. However, note in this last case 
we only need the exponents at $\infty$ for the computation to be provably correct.
\end{rem}
