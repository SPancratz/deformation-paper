...
\begin{thm} If $X$ is a smooth proper of dimension $m$, 
...
(and $\Frob_q$). For the construction and properties of these spaces we refer to [[TODO Jan]].
\end{defn}

\begin{thm} If $X$ is a smooth proper algebraic variety over $\FF_q$ of dimension $m$, 
then
\[
...
$\nabla:H^i(\mathfrak{X}/\mathfrak{S}) \rightarrow \Omega^1_{\mathfrak{S}} \otimes H^i(\mathfrak{X}/\mathfrak{S})$ is now defined as 
the differential $d_1: E_1^{0,i} \rightarrow E_1^{1,i}$ in this spectral sequence.
\end{defn}
...
Now suppose that $\mathfrak{E}$ is a vector bundle with connection on some Zariski open subset $\mathfrak{S}$ 
of $\mathbf{P}^1_{\QQ_q}$ with complement $\mathfrak{Z}$. Let $V$ denote the rigid 
...
For any $\tau \in S(\FF_{\mathfrak{q}})$, where $\mathfrak{q}=q^f$ for some $f \in \NN$,
\[
...
$\mathcal{X}/\mathcal{S}$ of smooth hypersurfaces contained in $\mathbf{P}^n_{\mathcal{S}}$. Now let
$\mathcal{U}/\mathcal{S}$ denote the complement of $\mathcal{X}/\mathcal{S}$ in $\mathbf{P}^n_{\mathcal{S}}$,
and write $\mathfrak{X}=\mathcal{X} \otimes \QQ_q$, $\mathfrak{U}=\mathcal{U} \otimes \QQ_q$, 
$\mathfrak{S}=\mathcal{S} \otimes \QQ_q$, and
$X=\mathcal{X} \otimes \FF_q$, $U=\mathcal{U} \otimes \QQ_q$, $S=\mathcal{S} \otimes \FF_q$. 


\begin{thm} For all $\tau \in S(\FF_{\mathfrak{q}})$, where $\mathfrak{q}=q^f$ for some $f \in \NN$, we have that
\begin{equation*}
...
on $\Hrig^n(U_{\tau})$. Moreover, $\chi(T)$ has degree 
\begin{equation*}
...
\]
Let $r \in \ZZ_q[t]$ be the denominator of $M$ i.e., such that $M=G/r$ with $G \in M_{b \times b}(\QQ_q[t])$. 
...
Let $\Phi$ denote the matrix of $p^{-1}\Frob_p$ with respect to the basis $[e_1, \ldots, e_b]$ i.e.,
\[
p^{-1} F_p (e_j) = \sum_{i=1}^b \Phi_{i,j} e_i.
\]

\begin{thm} The matrix $\Phi$ is an element of $M_{b \times b}(\QQ_q \langle t, \frac{1}{r(t)}) \rangle^{\dag}$ 
...
%TODO the following definition and theorem will depend on the basis, and should therefore probably
%be put after the definition of our basis \cB......
