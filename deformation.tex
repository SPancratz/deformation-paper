...
the matrices~$\Delta_k^{-1}$ once. After this, every call to {\sc{Decompose}} 
...
contain a lot of zeros, methods from sparse linear algebra can be used to compute such
a decomposition. After this, every call to {\sc{Decompose}} reduces to solving a 
...
that is available at \url{https://github.com/SPancratz/deformation}. All computations were carried
out on a laptop with two Intel Core i7-3540M processors running at 3GHz and with 8Gb of RAM, but all
computations were carried out on a single processor and maximum memory usage was about 1.5 Gb. Timings
are obtained using the C function clock() and are stated in minutes (m) or seconds (s). We are still
working on the implementation and hope to include updated timings in the final version of this paper.
...
\begin{rem}
Our timings in this example are about 5000 times lower than that of Gerkmann. 
\end{rem}
...
similar to the one for the matrix $C$!). Also, he seems to have made some mistakes in 
the precision analysis. For example, for the quartic surface his final 
$p$-adic precision does not seem to be high enough to recover the exact zeta function with 
...
that he could not compute the connection matrix for a quartic surface given by a polynomial 
with more than a \emph{few} nonzero terms. This was the main motivation for the work in the Phd-thesis 
of the first author and the present paper. We are now able to compute the zeta function of
...
We have not been able to compute the exponents of $M$ at the zeros of $r_1,r_2,r_3$ using 
MAGMA.
...
brings the degree of $s$ down to $8833$. Note that this does require factorizing the polynomials $\det(\Delta_2)$, 
$\det(\Delta_3)$, $\det(\Delta_4)$ over $\FF_7[t]$ and lifting a couple of factors to $\ZZ_7[t]$, but this takes 
almost no time using MAGMA. We can apply Theorem~\ref{thm:KedlayaTuitman} at $\infty$ to find $\theta_{\infty}=-4$,
