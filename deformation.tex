...
$K \in \NN$, such that $D$ divides some power of $r$,
and $D \Phi$ is congruent modulo $p^{N_{\Phi}}$ to a matrix of 
polynomials of degree less than $K$.
\end{thm}
...
a pole at $z$, i.e. if the polynomial $r$ does not have a zero at $z$. We now define 
\begin{align*}
D &= \prod_{z \neq \infty, \ord_p(z) \geq 0} (t-z)^{\gamma_z} 
    \prod_{z \neq \infty, \ord_p(z) < 0}(\frac{t}{z}-1)^{\gamma_z}, 
&K&= \bigl(\sum_{z} \gamma_z \bigr) +1,    
\end{align*}
which clearly satisfy all the conditions.
\end{proof}
...
\item Determine $K \in \NN$ and $D \in \ZZ_q[t]$ from Corollary \ref{thm:defD}.
